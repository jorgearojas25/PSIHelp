...
const CustomDrawer = props => {
  ...
  return (
    <Animated.View style={[styles.layout.simpleView, {opacity: opacity}]}>
      <SafeAreaView style={styles.layout.imageContainer} edges={['top']}>
        <ImageBackground
          source={{
          ...
          style={styles.components.drawerImage}
          imageStyle={styles.components.imageStyle}
          resizeMode="cover">
          <LinearGradient
            colors={['transparent', 'rgba(0,0,0,0.5)', 'rgba(0,0,0,0.7)']}
            style={styles.components.imageGradient}
          />
        </ImageBackground>
      </SafeAreaView>
      <DrawerContentScrollView
        {...props}
        contentContainerStyle={styles.layout.paddingTop}>
        {routes.map((route, position) => {
          ...
          return (
            <DrawerItem
              ...
              label={({focused}) => {
                return (
                  <Text
                    style={
                      focused
                        ? styles.state.activeText
                        : styles.state.inactiveText
                    }>
                    {route.name}
                  </Text>
                );
              ...
              style={
                isFocused
                  ? styles.state.activeContainer
                  : styles.state.inActiveContainer
              }
